...
def get_possible_umap_params() -> list:
  n_neighbor_params, base_n_neighbor, n_neighbor_multiplier = [], 3, 1
  n_components_params, base_n_components, n_components_multiplier = [], 2, 1
  min_dist_params, base_min_dist, min_dist_multiplier = [], 0.05, 0.05
...
def get_possible_hdbscan_params() -> list:
  min_cluster_size_params, base_min_cluster_size, min_cluster_size_multiplier = [], 15, 5
  min_samples_size_params, base_min_samples, min_samples_multiplier = [], 15, 5
  for increment in range(0, 5):
...
if __name__ == '__main__':
    with open('./data.xlsx', 'rb') as descriptions:
        ...
        sentences = [token for text in texts for token in tokenizer.tokenize(str(text))[1:]]
        preprocessed_sentences = list(map(lambda sentence: preprocess_text(sentence), sentences))
        ...
        for umap_index, umap_param in enumerate(umap_params):
            for hdbscan_index, hdbscan_param in enumerate(hdbscan_params):
                ...
                file_path = './topics/umap_{0}_hdbscan_{1}.txt'.format(umap_index, hdbscan_index)
